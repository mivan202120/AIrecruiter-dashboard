...
export const ProcessMetrics = ({ data }: ProcessMetricsProps) => {
  ...
  const fastestConversation = data.candidates.reduce((fastest, current) =>
    current.conversationMetrics.duration < fastest.conversationMetrics.duration ? current : fastest
  )

  const longestConversation = data.candidates.reduce((longest, current) =>
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: formatDuration(fastestConversation.conversationMetrics.duration),
      sublabel: fastestConversation.candidateName,
      icon: '🚀',
    },
    {
      label: 'Longest Conversation',
      value: formatDuration(longestConversation.conversationMetrics.duration),
